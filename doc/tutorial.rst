*****************************
Getting started with PyWOFOST
*****************************

This tutorial gives some examples to get you started with the PyWOFOST
package. Moreover, it gives some information on how to define your own
data sources and/or parameter files. 
All tests and examples are developed with the Enthought Python Distribution
(http://www.enthought.com/products/epd.php) under windows7 (EPD 32bit).

Testing the PyWOFOST package
============================
To guarantee its integrity, the PyWOFOST package includes a number of self
...
in the `pywofost.test_data` package, while the test data for the entire chain
is stored in a database (pywofost.db) that can be found under
`pywofost.database`.

We assume here that PyWOFOST is installed under 'D:\\USERDATA\\pylib\\' and
...
Next, PyWOFOST can be imported and the tests can be executed by calling
the `test()` function at the top of the package::

    >>> import pywofost
    >>> pywofost.test()
    runTest (pywofost.tests.test_abioticdamage.Test_FROSTOL) ... ok
    runTest (pywofost.tests.test_assimilation.Test_WOFOST_Assimilation) ... ok
    runTest (pywofost.tests.test_partitioning.Test_DVS_Partitioning) ... ok
    runTest (pywofost.tests.test_evapotranspiration.Test_PotentialEvapotranspiration) ... ok
    runTest (pywofost.tests.test_evapotranspiration.Test_WaterLimitedEvapotranspiration1) ... ok
    runTest (pywofost.tests.test_evapotranspiration.Test_WaterLimitedEvapotranspiration2) ... ok
    runTest (pywofost.tests.test_respiration.Test_WOFOSTMaintenanceRespiration) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialPotato) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedSunflower) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedSpringBarley) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedPotato) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialWinterRapeseed) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialGrainMaize) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialWinterWheat) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedWinterWheat) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedGrainMaize) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialSpringBarley) ... ok
    runTest (pywofost.tests.test_pywofost.TestWaterlimitedWinterRapeseed) ... ok
    runTest (pywofost.tests.test_pywofost.TestPotentialSunflower) ... ok

    ----------------------------------------------------------------------
    Ran 19 tests in 24.195s
...
An interactive PyWOFOST session
===============================
The easiest way to demonstrate PyWOFOST is to run it from an interactive
Python session using the built-in demo database which contains
meteorologic data, soil data and crop data for a grid location in South-Spain.

Initializing PyWOFOST and advancing model state
-----------------------------------------------
Let's start a PyWOFOST object for modelling winter-wheat (crop=1) on a
location in South-Spain (grid 31031) for the year 2000 under water-limited
conditions (mode='wlp')::

    >>> wofost_object = pywofost.start_pywofost(grid=31031, crop=1, year=2000, mode='wlp')
    >>> type(wofost_object)
    <class 'pywofost.pywofost.PyWofost'>

You have just succesfully initialized a WOFOST object in the python interpreter,
which is in its initial state and waiting to do some modelling. We
...
can retrieve any state or rate variable that is defined somewhere in a
SimulationObject. 
...
and store the results to a file 'myresults.csv'::
    ...
    >>> wofost_object.store_to_file("myresults.csv")

Which should look like this :download:`myresults.csv`

Alternatively, you can store the simulation results into a database using the
`store_to_database()` method.  

Using PyWOFOST with your own data
=================================

After running the examples you may be wondering where the data come
from that are used to run PyWOFOST. In fact, these data are retrieved from
an SQLite database 'pywofost.db' that is included with the source distribution
and can be found in the database folder.

For setting up PyWOFOST with your
own datasources you should understand that WOFOST uses 5 different types of
inputs: `cropdata`, `soildata`, `timerdata`, `sitedata` and driving variables
(i.e. weather data). The fact that these are called `\*data` is a bit of
...
All the input data types must be provided as python dictionaries
storing key/value pairs and several tools are available in the PyWOFOST
distribution to read these from a file or a database.
...
components (SimulationObjects) for running the crop simulation. These are
...
ways of setting up the cropdata:
    ...
    1. The WOFOST crop definition files are provided with the WOFOST Control Centre
       ...
       return a dictionary object that is directly useable by PyWOFOST.
