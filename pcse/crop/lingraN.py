...
class SourceLimitedGrowth(SimulationObject):
    ...
    *External dependencies:*
    ...
    RFTRA             Reduction factor on light use       pcse.crop.Evapotranspiration
                      effiency due to water stress
    RFNUTR            Reduction factor on light use       pylingra.Nstress
                      efficiency due to nutrient stress
    ===============  =================================== ==============================
    """
    ...
    def __call__(self, day, drv):
        ...
        # LUE corrected for transpiration or nutrient stress
...
class SinkLimitedGrowth(SimulationObject):
    ...
    *External dependencies:*
    ...
    def _calc_tillering_rate(self, LeafAppRate):
        k = self.kiosk
        p = self.params
        # Maximum site filling new buds decreases due
...
class LINGRA_N(SimulationObject):
    ...
    *External dependencies:*
    ...
    def calc_rates(self, day, drv):
        ...
        # rate plus maximum of death rates of shading, drought and nitrogen, d-1
        RDRtotal = p.RDRbase + max(RDRshading, RDRdrought, RDRnitrogen)
        # Actual death rate of leaf area, due to relative death rate
        # of leaves or rate of change due to cutting, ha ha-1 d-1
        if self._flag_MOWING:
