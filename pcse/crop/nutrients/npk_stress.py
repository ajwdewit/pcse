...
class NPK_Stress(SimulationObject):
    ...
    **External dependencies:**
    ...
    def __call__(self, day):
        ...
        NCRITLV  = params.NCRIT_FR * NMAXLV * WLV
        NCRITST  = params.NCRIT_FR * NMAXST * WST
        
        PCRITLV = params.PCRIT_FR * PMAXLV * WLV
        PCRITST = params.PCRIT_FR * PMAXST * WST

        KCRITLV = params.KCRIT_FR * KMAXLV * WLV
        KCRITST = params.KCRIT_FR * KMAXST * WST
        ...
        if (NCRITMR - NRMR) > 0.:
            NNI = max(0.001, (NFGMR-NRMR)/(NCRITMR-NRMR))
        else:
            NNI = 0.001
            
        if (PCRITMR - PRMR) > 0.:
            PNI = max(0.001, (PFGMR-PRMR)/(PCRITMR-PRMR))
        else:
           PNI = 0.001
            
        if (KCRITMR-KRMR) > 0:
            KNI = max(0.001, (KFGMR-KRMR)/(KCRITMR-KRMR))
        else:
            KNI = 0.001
