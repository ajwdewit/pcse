...
Modifications 2012/06/28 by Allard de Wit
- on_trait_change handler is now executed when a value is assigned to a trait
  not only when the new value is different from the current one.
- A new trait has been defined 'AfgenTrait', to define Afgen tables as
  WOFOST parameters.
"""
...
# Import needed for addition of AfgenTrait type
...
class TraitType(object):
    ...
    # Modification Allard de Wit 2012/02/24
    # notify handler is now executed on assignment instead of change of value
