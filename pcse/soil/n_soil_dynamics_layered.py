...
class N_soil_dynamics_layered(SimulationObject):
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        NAVAIL = sinm.calculate_NAVAIL(self.soiln_profile, p.KSORP, s.NH4, s.NO3, RD_m, SM, TRALYIL, p.TSCF_N) / self.m2_to_ha
    ...
    def integrate(self, day, delt=1.0):
        ...
        s.NAVAIL = sinm.calculate_NAVAIL(self.soiln_profile, p.KSORP, s.NH4, s.NO3, RD_m, SM, TRALYIL, p.TSCF_N) / self.m2_to_ha
    ...
    class SoilInorganicNModel():
        ...
        def calculate_NAVAIL(self, soiln_profile, KSORP, NH4, NO3, RD_m, SM, TRALYIL, TSCF_N):
            samm = self.SoilAmmoniumNModel()
            sni = self.SoilNNitrateModel()
            zmin = 0.
            NAVAIL = 0.
            for il, layer in enumerate(soiln_profile):
                ...
                NH4_avail_layer = samm.calculate_available_NH4(KSORP, NH4[il], RD_m, layer.RHOD_kg_per_m3, SM[il], TRALYIL, TSCF_N, zmax, zmin)
                NO3_avail_layer = sni.calculate_available_NO3(NO3[il], RD_m, TRALYIL, TSCF_N, zmax, zmin)     
                NAVAIL += (NH4_avail_layer + NO3_avail_layer)
                zmin = zmax
            return NAVAIL

        def calculate_N_uptake_rates(self, soiln_profile, delt, KSORP, N_demand_soil, NH4, NO3, RD_m, SM, TRALY, TSCF_N):
            ...
            for il, layer in enumerate(soiln_profile):
                zmax = zmin + layer.Thickness_m
                RNH4UP[il] = samm.calculate_NH4_plant_uptake_rate(KSORP, N_demand_soil, NH4[il], RD_m, layer.RHOD_kg_per_m3, SM[il], TRALY[il], TSCF_N, zmax, zmin)
                N_demand_soil-= RNH4UP[il] * delt
                RNO3UP[il] = sni.calculate_NO3_plant_uptake_rate(N_demand_soil, NO3[il], RD_m, TRALY[il], TSCF_N, zmax, zmin)
        ...
        class SoilNNitrateModel():
            ...
            def calculate_available_NO3(self, NO3, RD, TRALYIL, TSCF_N, zmax, zmin):
                dz = zmax - zmin
                if(RD <= zmin):
                    NO3_avail_layer = 0.
                elif(RD > zmax):
            ...
            def calculate_NO3_plant_uptake_rate(self, N_demand_soil, NO3, RD_m, TRALYIL, TSCF_N, zmax, zmin):
                NO3_av = self.calculate_available_NO3(NO3, RD_m, TRALYIL, TSCF_N, zmax, zmin)
